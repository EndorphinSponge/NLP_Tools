...
def refineAbrvs(json_path: Union[str, bytes, os.PathLike],
    ...
    trans_final = {item[0][0]: item[0][1] for item in trans_counter.items()}
    
        
    with open(f"{root_name}_trans.json", "w") as file:
        # counter_json: list[tuple[tuple[str, str], int]] = trans_counter.items() # Gives list of tuples of the counted object (tuple[str, str]) and its count
