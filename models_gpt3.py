...
df_slice.to_excel("extracted.xlsx")

#%% Execution ACTIVE

df_slice = DATA[1050:].reset_index(drop = True) # Start bound is included, end bound is not
df_new = pd.DataFrame() # Don't need to initiate columns since it is done automatically when adding entries
for index, row in df_slice.iterrows():
    print(index)
    abstract = row["Abstract"]
...
#%% Read single object from output
